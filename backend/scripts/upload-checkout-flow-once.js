#!/usr/bin/env node
// Upload + publish checkout-flow.json to WHATSAPP_CHECKOUT_FLOW_ID.
require('dotenv').config({ path: require('path').join(__dirname, '../.env.local') });
const { spawnSync } = require('child_process');
const path = require('path');

const id = process.env.WHATSAPP_CHECKOUT_FLOW_ID;
const token = process.env.WHATSAPP_ACCESS_TOKEN;
const file = path.join(__dirname, '../src/flows/checkout-flow.json');

if (!id || !token) {
  console.error('Missing WHATSAPP_CHECKOUT_FLOW_ID or WHATSAPP_ACCESS_TOKEN');
  process.exit(1);
}
...
console.log('Uploaded and published checkout Flow', id);
